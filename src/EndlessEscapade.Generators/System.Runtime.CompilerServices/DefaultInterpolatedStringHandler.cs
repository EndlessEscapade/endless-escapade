// Copied from https://github.com/dotnet/runtime/blob/5535e31a712343a63f5d7d796cd874e563e5ac14/src/libraries/System.Private.CoreLib/src/System/Runtime/CompilerServices/DefaultInterpolatedStringHandler.cs
// and modified
...
public ref struct DefaultInterpolatedStringHandler
{
    ...
    /// <remarks>
    ///     This is intended to be called only by compiler-generated code. Arguments are not validated as they'd otherwise be for members intended to be used
    ///     directly.
    /// </remarks>
    ...
    public string ToStringAndClear() {
        var result = Text.ToString(); //new string(Text);
        Clear();
        return result;
    }
    ...
    ///     Minimum number of characters that should be written for this value.  If the value is negative, it indicates left-aligned and the
    ///     required minimum is the absolute value.
    /// </param>
    /// <typeparam name="T">The type of the value to write.</typeparam>
    ...
    public void AppendFormatted(string? value) {
        ...
        if (!_hasCustomFormatter && value is not null && value.AsSpan().TryCopyTo(_chars.Slice(_pos))) {
            _pos += value.Length;
        }
        else {
    ...
    /// <param name="alignment">
    ///     Non-zero minimum number of characters that should be written for this value.  If the value is negative, it indicates left-aligned and
    ///     the required minimum is the absolute value.
    /// </param>
    ...
    private void GrowCore(uint requiredMinCapacity) {
        ...
        // EERemark: The maximum string length in .net 8 coreclr is 0x3FFFFFDF but this is an implementation detail
        var newCapacity = Math.Max(
            requiredMinCapacity,
            Math.Min((uint)_chars.Length * 2, int.MaxValue)
        ); //Math.Max(requiredMinCapacity, Math.Min((uint)_chars.Length * 2, string.MaxLength));
        var arraySize = unchecked((int)Math.Min(
            Math.Max(newCapacity, MinimumArrayPoolLength),
            int.MaxValue
        )); //(int)Math.Clamp(newCapacity, MinimumArrayPoolLength, int.MaxValue);
