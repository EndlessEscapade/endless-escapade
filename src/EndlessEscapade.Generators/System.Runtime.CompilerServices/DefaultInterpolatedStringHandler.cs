...
using EndlessEscapade.Generators;
...
namespace System.Runtime.CompilerServices
{
    ...
    public ref struct DefaultInterpolatedStringHandler
    {
        ...
        public override string ToString() => Text.ToString();//new string(Text);
        ...
        {
            string result = Text.ToString();//new string(Text);
            Clear();
            return result;
        }
        ...
        private void GrowCore(uint requiredMinCapacity)
        {
            ...
            uint newCapacity = Math.Max(requiredMinCapacity, Math.Min((uint)_chars.Length * 2, int.MaxValue)); //Math.Max(requiredMinCapacity, Math.Min((uint)_chars.Length * 2, string.MaxLength));
            int arraySize = unchecked((int)Math.Min(Math.Max(newCapacity, MinimumArrayPoolLength), int.MaxValue)); //(int)Math.Clamp(newCapacity, MinimumArrayPoolLength, int.MaxValue);
