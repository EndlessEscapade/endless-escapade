    ...
    public class EEPlayer : ModPlayer
    {
        ...
        public override void UpdateBiomeVisuals()
        {
            ...
            else if (Main.ActiveWorldFileData.Name == key2)
            {
                ...
                if (!arrowFlag)
                {
                    Anchors = Projectile.NewProjectile(player.Center, Vector2.Zero, mod.ProjectileType("Anchor"), 0, 0, player.whoAmI, pos3X, pos3Y);
                    AnchorsVolc = Projectile.NewProjectile(player.Center, Vector2.Zero, mod.ProjectileType("Anchor"), 0, 0, player.whoAmI, pos2X, pos2Y - 50);
                    arrowFlag = true;
                }
                if (isNearIsland)
                {
                    ...
                    for (int i = 0; i < 400; i++)
                    {
                        ...
                        if (dist.Length() > 1140)
                        {
                            switch (CloudChoose)
                            {
                                case 0:
                                    {
                                        Projectile.NewProjectile(CloudPos, Vector2.Zero, mod.ProjectileType("DarkCloud1"), 0, 0f, Main.myPlayer, Main.rand.NextFloat(0.6f, 1f), Main.rand.Next(60, 180));
                                        break;
                                    }
                                case 1:
                                    {
                                        Projectile.NewProjectile(CloudPos, Vector2.Zero, mod.ProjectileType("DarkCloud2"), 0, 0f, Main.myPlayer, Main.rand.NextFloat(0.6f, 1f), Main.rand.Next(60, 180));
                                        break;
                                    }
                                case 2:
                                    {
                                        Projectile.NewProjectile(CloudPos, Vector2.Zero, mod.ProjectileType("DarkCloud3"), 0, 0f, Main.myPlayer, Main.rand.NextFloat(0.6f, 1f), Main.rand.Next(60, 180));
                                        break;
                                    }
                            }
                        }
            ...
            else
            {
                ...
                {
                    Arrow = Projectile.NewProjectile(player.Center, Vector2.Zero, mod.ProjectileType("DesArrowProjectile"), 0, 0, player.whoAmI);
                    Arrow2 = Projectile.NewProjectile(player.Center, Vector2.Zero, mod.ProjectileType("OceanArrowProjectile"), 0, 0, player.whoAmI);
                    arrowFlag = true;
                }
                if (EEWorld.EntracesPosses.Count > 0)
                {
                    if (player.Center.X / 16 >= (EEWorld.EntracesPosses[0].X + 12) - 2 &&
                        player.Center.X / 16 <= (EEWorld.EntracesPosses[0].X + 12) + 2 &&
                        player.Center.Y / 16 >= (EEWorld.EntracesPosses[0].Y + 7) - 2 &&
                        player.Center.Y / 16 <= (EEWorld.EntracesPosses[0].Y + 7) + 2 &&
                            EEWorld.EntracesPosses.Count > 0)
                    {
                        if (player.controlUp)
                        {
                            godMode = true;
