...
namespace EEMod.Items
{
    public class TennisRachetProj : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            int projs = Main.projectile.Length - 1;
            for (int i = 0; i < projs; i++)
            {
                Projectile proj = Main.projectile[i];
                if (proj.type == ModContent.ProjectileType<TenisBoll>()
                    && proj.active
                    && (proj.Center - projectile.Center).LengthSquared() < (proj.Center - pos).LengthSquared())
                {
                    gottenBoll = proj;
                }
            }
