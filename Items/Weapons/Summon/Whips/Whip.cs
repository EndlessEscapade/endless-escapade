    ...
    public abstract class Whip : EEProjectile
    {
        ...
        private void ApplyNPCOnHitEffects(Item sItem, Rectangle itemRectangle, int damage, float knockBack, int npcIndex, int dmgRandomized, int dmgDone)
        {
            ...
            if (Main.npc[npcIndex].value > 0f && Main.rand.Next(5) == 0)
            {
                ...
                int Coin = Item.NewItem(new Terraria.DataStructures.EntitySource_Parent(Projectile), (int)Main.npc[npcIndex].position.X, (int)Main.npc[npcIndex].position.Y, Main.npc[npcIndex].width, Main.npc[npcIndex].height, type);
