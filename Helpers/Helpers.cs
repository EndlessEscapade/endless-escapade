            ...
            for (float i = 0; i <= 1; i += chainsPerUse)
            {
                ...
                if (i != 0)
                {
                    ...
                    if (emitsDust)
                    {
                        ...
                        {
                            if (!Framing.GetTileSafely((int)x / 16, (int)y / 16).HasTile)
                            {
        ...
        public static void DrawBezierBreakOnTiles(SpriteBatch spriteBatch, Texture2D headTexture, string glowMaskTexture, Color drawColor, Vector2 endPoints, Vector2 startingPos, Vector2 c1, Vector2 c2, float chainsPerUse, float rotDis, Texture2D endingTexture)
        {
            for (float i = 0; i <= 1; i += chainsPerUse)
            {
                ...
                if (i != 0)
                {
                    ...
                    if (Framing.GetTileSafely((int)(X(i, startingPos.X, c1.X, c2.X, endPoints.X) / 16f), (int)(Y(i, startingPos.Y, c1.Y, c2.Y, endPoints.Y) / 16f)).HasTile) break;
        ...
        public static void DrawBezierProj(Vector2 endPoints, Vector2 startingPos, Vector2 c1, Vector2 c2, float chainsPerUse, float rotDis, int projType, bool isBridge)
        {
            ...
            for (float i = 0; i <= 1; i += chainsPerUse)
            {
                ...
                if (i != 0)
                {
                    ...
                    int proj = Projectile.NewProjectile(new Terraria.DataStructures.EntitySource_Misc(""), new Vector2(X(i, startingPos.X, c1.X, c2.X, endPoints.X), Y(i, startingPos.Y, c1.Y, c2.Y, endPoints.Y)), Vector2.Zero, projType, 0, 0f, Main.myPlayer, 0, i);
