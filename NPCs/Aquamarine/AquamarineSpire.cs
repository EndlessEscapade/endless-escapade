        ...
        public override void AI()
        {
            ...
            if (awake)
            {
                ...
                if (NPC.ai[2] == 0 && ((specialLaserShots < 3 && NPC.ai[1] == 1) || (specialLaserShots < 2 && NPC.ai[1] == 3) || (NPC.ai[1] != 3 && NPC.ai[1] != 1)))
                {
                    ...
                    {
                        case 0: //Blue laser
                            Projectile projectile = Projectile.NewProjectileDirect(new EntitySource_Parent(NPC), eyePos, Vector2.Normalize(target.Center - NPC.Center) * 2, ModContent.ProjectileType<SpireLaser>(), NPC.damage, 0f, default, 0, 1);
                        ...
                        case 1: //Pink laser
                            Projectile projectile2 = Projectile.NewProjectileDirect(new EntitySource_Parent(NPC), eyePos, Vector2.Normalize(target.Center - NPC.Center) * 2, ModContent.ProjectileType<SpireLaser>(), NPC.damage / 2, 0f, default, 0, 2);
                        ...
                        case 2: //White laser
                            Projectile projectile3 = Projectile.NewProjectileDirect(new EntitySource_Parent(NPC), eyePos, Vector2.Normalize(target.Center - NPC.Center) * 2, ModContent.ProjectileType<WideSpireLaser>(), NPC.damage / 2, 0f, default, 0, 3);
                        ...
                        case 3: //Cyan laser
                            ...
                            {
                                Projectile projectile4 = Projectile.NewProjectileDirect(new EntitySource_Parent(NPC), eyePos, Vector2.Normalize(target.Center - NPC.Center) * 2, ModContent.ProjectileType<SpireLaser>(), NPC.damage / 3, 0f, default, 0, 1);
                            ...
                            if (specialLaserShots == 1)
                            {
                                for (int i = -1; i < 2; i += 2)
                                {
                                    Projectile projectile4 = Projectile.NewProjectileDirect(new EntitySource_Parent(NPC), eyePos, (Vector2.Normalize(target.Center - NPC.Center)).RotatedBy(i / 6f) * 2, ModContent.ProjectileType<SpireLaser>(), NPC.damage / 3, 0f, default, 0, 4);
                                    PrimitiveSystem.primitives.CreateTrail(new SpirePrimTrail(projectile4, Color.Lerp(Color.LightCyan, Color.DarkCyan, Main.rand.NextFloat(0, 1)), 10));
                                }
                                specialLaserShots++;
                                NPC.ai[2] = 1;
                ...
                if (firstAwakening)
                {
                    ...
                    for (int i = (int)(NPC.Center.X / 16) - 150; i < (int)(NPC.Center.X / 16) + 150; i++)
                    {
                        for (int j = (int)(NPC.Center.Y / 16) - 150; j < (int)(NPC.Center.Y / 16) + 150; j++)
                        {
                            ...
                            {
                                Projectile proj = Projectile.NewProjectileDirect(new EntitySource_Parent(NPC), new Vector2((i * 16) + 16, (j * 16) - 12), Vector2.Zero, ModContent.ProjectileType<AquamarineLamp1Glow>(), 0, 0, default, 0, 0);
                                if (shields.Count < 10)
                                {
                                    shields.Add(proj);
                                }
