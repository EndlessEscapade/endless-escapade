...
namespace EEMod.NPCs.KelpForest.Kelpweaver
{
    public class GlowingKelpSpider : EENPC
    {
        ...
        public void DrawSpiderPort(Color drawColour)
        {
            ...
            for (int i = 0; i < numberOfLegs; i++)
            {
                ...
                Main.spriteBatch.Draw(UpperLegTexGlow, ((SpiderBodyPosition + jointPoints[i]) / 2f).ForDraw(), UpperLegTexGlow.Bounds, Color.White * lerpCache, (SpiderBodyPosition - jointPoints[i]).ToRotation() + (float)Math.PI / 2f, UpperLegTexGlow.TextureCenter(), 1f, SpriteEffects.None, 0f);

                Helpers.DrawBloomed(UpperLegTexGlow, ((SpiderBodyPosition + jointPoints[i]) / 2f).ForDraw(), Color.White * lerpCache, 1f, (SpiderBodyPosition - jointPoints[i]).ToRotation() + (float)Math.PI / 2f, 1f, 2.0f);

                if (i % 2 == 0)
                {
                    Main.spriteBatch.Draw(LowerLegTexGlow, ((legPoints[i] + jointPoints[i]) / 2f).ForDraw(), LowerLegTexGlow.Bounds, Color.White * lerpCache, (legPoints[i] - jointPoints[i]).ToRotation() + (float)Math.PI / 2f, LowerLegTexGlow.TextureCenter(), 0.9f, SpriteEffects.None, 0f);

                    Helpers.DrawBloomed(LowerLegTexGlow, ((legPoints[i] + jointPoints[i]) / 2f).ForDraw(), Color.White * lerpCache, 1f, (legPoints[i] - jointPoints[i]).ToRotation() + (float)Math.PI / 2f, 1f, 2.0f);
                }
            }
            ...
            Main.spriteBatch.Draw(KelpSpiderBodyGlow, SpiderBodyPosition.ForDraw(), KelpSpiderBodyGlow.Bounds, Color.White * lerpCache, rotation, KelpSpiderBodyGlow.TextureCenter(), 1f, !cond ? SpriteEffects.FlipVertically : SpriteEffects.None, 0f);

            Helpers.DrawBloomed(KelpSpiderBodyGlow, SpiderBodyPosition.ForDraw(), Color.White * lerpCache, 1f, rotation, 1f, 2.0f, !cond ? SpriteEffects.FlipVertically : SpriteEffects.None);
            ...
            for (int i = 0; i < numberOfLegs; i++)
            {
                ...
                if (i % 2 == 1)
                {
                    ...
                    Helpers.DrawBloomed(LowerLegTexGlow, ((legPoints[i] + jointPoints[i]) / 2f).ForDraw(), Color.White * lerpCache, 0.9f, (legPoints[i] - jointPoints[i]).ToRotation() + (float)Math.PI / 2f, 1f, 2.0f);
                }
            }
        }
