...
namespace EEMod.NPCs.Bosses.Gallagar
{
    public class GBeam : ModProjectile // Thanks to Dan Yami for the code
    {
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Color lightColor)
        {
            ...
            laserLength -= (float)(projectileTexture.Height / 2 + beamEndTexture.Height) * projectile.scale;
            Vector2 value20 = projectile.Center + new Vector2(0, projectile.gfxOffY);
            value20 += projectile.velocity * projectile.scale * (float)projectileTexture.Height / 2f;
            if (laserLength > 0f)
            {
                float num229 = 0f;
                Microsoft.Xna.Framework.Rectangle rectangle7 = new Microsoft.Xna.Framework.Rectangle(0, 16 * (projectile.timeLeft / 3 % 5), beamTexture.Width, 16);
                while (num229 + 1f < laserLength)
                {
                    if (laserLength - num229 < (float)rectangle7.Height)
                    {
                        rectangle7.Height = (int)(laserLength - num229);
                    }
                    Main.spriteBatch.Draw(beamTexture, value20 - Main.screenPosition, new Microsoft.Xna.Framework.Rectangle?(rectangle7), color44, projectile.rotation, new Vector2((float)(rectangle7.Width / 2), 0f), new Vector2(Math.Min(chargeCounter, charge) / charge, 1f), SpriteEffects.None, 0f);
