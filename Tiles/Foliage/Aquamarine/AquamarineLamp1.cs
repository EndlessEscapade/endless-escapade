                ...
                if (active)
                {
                    ...
                    if ((spire.ai[0] <= 40 && spire.ai[0] > 20 && !dead)) //If in first phase and projectile is not inactive
                    {
                        ...
                        if(((Projectile.ai[0] + Projectile.ai[1]) * 120) % Main.GameUpdateCount == 0)
                        {
                            ...
                            Projectile projectile2 = Projectile.NewProjectileDirect(new EntitySource_Misc(""), Projectile.Center, Vector2.Normalize(target.Center - Projectile.Center) * 2, ModContent.ProjectileType<SpireLaserAlt>(), (int)(Main.npc[spire.whoAmI].damage / 5f), 0f, default, 0, 1);
