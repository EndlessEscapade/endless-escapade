...
namespace EEMod.Tiles.Foliage.KelpForest
{
    ...
    public class GroundGlowCoral3 : EETile
    {
        ...
        public override void PostDraw(int i, int j, SpriteBatch spriteBatch)
        {
            ...
            Tile tile = Framing.GetTileSafely(i, j);
            Helpers.DrawTileGlowmask(Mod.Assets.Request<Texture2D>("Tiles/Foliage/KelpForest/GroundGlowCoralGlow3").Value, i, j, Color.White * (float)(0.5f + (Math.Sin((i - (tile.frameX / 18f)) + (j - (tile.frameY / 18f)) + Main.GameUpdateCount / 60f) / 2f)));
        }
    }
}