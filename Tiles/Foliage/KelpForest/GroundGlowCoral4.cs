...
namespace EEMod.Tiles.Foliage.KelpForest
{
    public class GroundGlowCoral4 : EETile
    {
        ...
        public override void ModifyLight(int i, int j, ref float r, ref float g, ref float b)
        {
            ...
            if ((tile.frameY == 18 || tile.frameY == 36) || (tile.frameX == 18 && tile.frameY == 72) || (tile.frameX == 18 && tile.frameY == 108))
            {
                r = 0.25f;
                g = 0.2f;
                b = 0f;
        ...
        public override void PostDraw(int i, int j, SpriteBatch spriteBatch)
        {
            ...
            Tile tile = Framing.GetTileSafely(i, j);
            Helpers.DrawTileGlowmask(Mod.Assets.Request<Texture2D>("Tiles/Foliage/KelpForest/GroundGlowCoralGlow4").Value, i, j, Color.White * (float)(0.5f + (Math.Sin((i - (tile.frameX / 18f)) + (j - (tile.frameY / 18f)) + Main.GameUpdateCount / 60f) / 2f)));
        }
    }
}