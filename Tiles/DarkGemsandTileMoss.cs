...
namespace EEMod.Tiles
{
    public class DarkGemsandTileMoss : ModTile
    {
        ...
        void PlaceGroundGrass(int i, int j)
        {
            ...
            string tex = "Tiles/Foliage/KelpGrassLongMoss";
            ...
            for (int a = 0; a < noOfGrassBlades; a++)
            {
                ...
                if ((i + j * a * 2) % 2 != 0)
                    ModContent.GetInstance<EEMod>().TVH.AddElement(new Leaf(new Vector2(pos, j * 16), Chosen, 0f, Color.Lerp(Color.Yellow, Color.LightYellow, ((i + j + a * 3) % 4) / 4f), false,true));
                else
                {
                    ModContent.GetInstance<EEMod>().TVH.AddElement(new Leaf(new Vector2(pos - EEMod.instance.GetTexture(Chosen).Width, j * 16), Chosen, 0f, Color.Lerp(Color.Yellow, Color.LightYellow, ((i + j + a * 3) % 4) / 4f), true,true));
                }
            }
        }
