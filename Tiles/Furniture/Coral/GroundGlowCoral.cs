...
{
    public class GroundGlowCoralTE : ModTileEntity
    {
    ...
    public class GroundGlowCoral : ModTile
    {
        ...
        public override void PostDraw(int i, int j, SpriteBatch spriteBatch)
        {
            ...
            if (tile != null && tile.active() && tile.type == Type)
            {
                ...
                int index = ModContent.GetInstance<GroundGlowCoralTE>().Find(i - tile.frameX / 16, j - tile.frameY / 16);
                if (index == -1)
                {
                    return;
                }
                GroundGlowCoralTE TE = (GroundGlowCoralTE)TileEntity.ByID[index];
                for (int k = 0; k < 7; k++)
                {
                    Main.spriteBatch.Draw(mod.GetTexture("Tiles/Furniture/Coral/GroundGlowCoralGlow"), new Vector2(i * 16 - (int)Main.screenPosition.X - (width - 16f) / 2f, j * 16 - (int)Main.screenPosition.Y + offsetY) + zero + new Vector2(2, 0), new Rectangle(frameX, frameY, width, height), color * ((float)Math.Sin(TE.kayLerp) * 0.5f + 0.5f), 0f, default, 1f, SpriteEffects.None, 0f);
                }
            }
        }
