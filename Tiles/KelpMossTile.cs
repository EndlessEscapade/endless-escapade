    ...
    public class KelpMossTile : EETile
    {
        ...
        public override void PostDraw(int i, int j, SpriteBatch spriteBatch)
        {
            ...
            if(Main.rand.Next(4) == 1 && !Framing.GetTileSafely(i,j-1).active())
            {
                ...
                EEMod.MainParticles.SpawnParticles(new Vector2(i*16,j*16), -Vector2.UnitY,3, chosen, new SlowDown(0.98f), new RotateTexture(Main.rand.NextFloat(-0.03f, 0.03f)), new SetMask(ModContent.GetInstance<EEMod>().GetTexture("Textures/RadialGradient")), new AfterImageTrail(1f), new RotateVelocity(Main.rand.NextFloat(-0.02f,0.02f)), new SetLighting(chosen.ToVector3(),0.1f));
            }    
