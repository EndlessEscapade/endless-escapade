...
namespace EEMod.Projectiles.CoralReefs
{
    public class ThermalMagma : ModProjectile
    {
        ...
        public override void PostDraw(SpriteBatch spriteBatch, Color lightColor)
        {
            ...
            EEMod.Particles.Get("Main").SetSpawningModules(new SpawnRandomly(0.2f));
            EEMod.Particles.Get("Main").SpawnParticles(projectile.Center + new Vector2(Main.rand.Next(-17, 17), Main.rand.Next(-17, 17)), new Vector2(Main.rand.NextFloat(-1.25f, 1.25f), Main.rand.NextFloat(-1.25f, 1.25f)), mod.GetTexture("Particles/Cross"), 30, 1, Color.Lerp(Color.OrangeRed, Color.LightGoldenrodYellow, Main.rand.NextFloat(0f, 1f)), new SlowDown(0.97f), new SetMask(ModContent.GetInstance<EEMod>().GetTexture("Masks/RadialGradient")), new RotateVelocity(Main.rand.NextFloat(-0.01f, 0.01f)), new RotateTexture(0.02f), new AfterImageTrail(0.75f), new SetLighting(new Vector3(4f, 2f, 0f), 0.3f));
        }
    }
}