    ...
    public class EEParticle : EEProjectile
    {
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            for (int i = 0; i < Particles.Count; i++)
            {
                ...
                Main.spriteBatch.Draw(ModContent.Request<Texture2D>("EEMod/Projectiles/Particles").Value, Particles[i].Position - Main.screenPosition, null, Color.White * Math.Abs((float)Math.Sin(Particles[i].flash / (Particles[i].alpha / 3f))), Particles[i].flash / 10f, Vector2.Zero, Particles[i].scale, SpriteEffects.None, 0);
                Main.spriteBatch.End();
            ...
            /*for (int i = 0; i < Particles.Count; i++)
            {
                ...
                Main.spriteBatch.Draw(ModContent.Request<Texture2D>("Particles/ForegroundParticles/Leaf").Value, Leaves[i].Position - Main.screenPosition, null, Color.White, Leaves[i].Velocity.ToRotation() + Leaves[i].rotation, Vector2.Zero, Leaves[i].scale, SpriteEffects.None, 0);
            }*/
            return false;
        }
