    ...
    public class EEParticle : EEProjectile
    {
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            for (int i = 0; i < Particles.Count; i++)
            {
                ...
                spriteBatch.Draw(ModContent.GetInstance<EEMod>().GetTexture("Particles/ForegroundParticles/Leaf"), Leaves[i].Position - Main.screenPosition, null, Color.White, Leaves[i].Velocity.ToRotation() + Leaves[i].rotation, Vector2.Zero, Leaves[i].scale, SpriteEffects.None, 0);
            }
            return false;
        }
