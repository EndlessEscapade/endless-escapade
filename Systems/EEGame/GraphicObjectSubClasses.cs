...
namespace EEMod
{
    ...
    public class SeagullsClass : GraphicObject
    {
        public override void Draw(Texture2D tex, int noOfFrames, int PerFrame)
        {
            ...
            Color drawColour2 = Lighting.GetColor((int)((Position.X + Main.screenPosition.X) / 16f), (int)((Position.Y + Main.screenPosition.Y) / 16f)) * Main.LocalPlayer.GetModPlayer<EEPlayer>().seamapLightColor;
            drawColour2.A = 255;
            ...
            Color drawColour = Color.Black * (1 - (Main.LocalPlayer.GetModPlayer<EEPlayer>().cutSceneTriggerTimer / 180f)) * MathHelper.Clamp((0.5f * Lighting.GetColor((int)(Position.X / 16f), (int)(Position.Y / 16f)).A), 0.1f, 0.5f);
