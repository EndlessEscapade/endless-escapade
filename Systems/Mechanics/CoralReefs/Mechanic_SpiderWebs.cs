...
namespace EEMod
{
    public class SpiderWebs : Mechanic
    {
        ...
        void HandleWebDraw(Vector2 position)
        {
            ...
            if (p1.Y >= 1 && p5.Y >= 1)
                Helpers.DrawBezierAdditive(vineTexture, Lighting.GetColor((int)(p1Mid.X / 16), (int)(p1Mid.Y / 16)), p1Mid, p5Mid, Vector2.Lerp(p1Mid, p5Mid, 0.5f) + new Vector2(0, -40 + (float)Math.Sin(sineInt * 3) * 40), cockandbol, (float)Math.PI / 2, false, 1, false);

            if (p5.Y >= 1 && p6.Y >= 1)
                Helpers.DrawBezierAdditive(vineTexture, Lighting.GetColor((int)(p5Mid.X / 16), (int)(p5Mid.Y / 16)), p5Mid, p6Mid, Vector2.Lerp(p5Mid, p6Mid, 0.5f) + new Vector2(-40 + (float)Math.Sin(sineInt * 4) * 40, 0), cockandbol, (float)Math.PI / 2, false, 1, false);

            if (p6.Y >= 1 && p2.Y >= 1)
                Helpers.DrawBezierAdditive(vineTexture, Lighting.GetColor((int)(p6Mid.X / 16), (int)(p6Mid.Y / 16)), p6Mid, p2Mid, Vector2.Lerp(p6Mid, p2Mid, 0.5f) + new Vector2(0, 40 + (float)Math.Sin(sineInt * 3) * 40), cockandbol, (float)Math.PI / 2, false, 1, false);

            if (p2.Y >= 1 && p4.Y >= 1)
                Helpers.DrawBezierAdditive(vineTexture, Lighting.GetColor((int)(p2Mid.X / 16), (int)(p2Mid.Y / 16)), p2Mid, p4Mid, Vector2.Lerp(p2Mid, p4Mid, 0.5f) + new Vector2(0, 40 + (float)Math.Sin(sineInt * 4) * 40), cockandbol, (float)Math.PI / 2, false, 1, false);

            if (p4.Y >= 1 && p3.Y >= 1)
                Helpers.DrawBezierAdditive(vineTexture, Lighting.GetColor((int)(p4Mid.X / 16), (int)(p4Mid.Y / 16)), p4Mid, p3Mid, Vector2.Lerp(p4Mid, p3Mid, 0.5f) + new Vector2(40 + (float)Math.Sin(sineInt * 3) * 40, 0), cockandbol, (float)Math.PI / 2, false, 1, false);

            if (p3.Y >= 1 && p1.Y >= 1)
                Helpers.DrawBezierAdditive(vineTexture, Lighting.GetColor((int)(p3Mid.X / 16), (int)(p3Mid.Y / 16)), p3Mid, p1Mid, Vector2.Lerp(p3Mid, p1Mid, 0.5f) + new Vector2(0, -40 + (float)Math.Sin(sineInt * 4) * 40), cockandbol, (float)Math.PI / 2, false, 1, false);
        }
    }
}