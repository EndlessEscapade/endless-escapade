...
namespace EEMod
{
    public class BulbousPlants : ModSystem
    {
        private void HandleBulbDraw(Vector2 position)
        {
            ...
            EEMod.Noise2DShift.Parameters["noiseTexture"].SetValue(EEMod.Instance.Assets.Request<Texture2D>("Textures/Noise/noise").Value);
            EEMod.Noise2DShift.Parameters["tentacle"].SetValue(EEMod.Instance.Assets.Request<Texture2D>("Textures/Noise/WormNoisePixelated").Value);
            ...
            Texture2D tex = EEMod.Instance.Assets.Request<Texture2D>("Textures/BulbousBall").Value;
