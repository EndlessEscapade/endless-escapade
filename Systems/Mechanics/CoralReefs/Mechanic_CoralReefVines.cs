                    ...
                    if (Vector2.DistanceSquared(ChainConneccPos, LastChainConneccPos) < 100 * 16 * 100 * 16)
                    {
                        ...
                        if (MidNorm.Y > 100 * 16 && Vector2.DistanceSquared(Main.LocalPlayer.Center, MidNorm) < 2000 * 2000)
                        {
                            ...
                            if (isValid && Collision.CanHit(lerp1, 1, 1, lerp2, 1, 1))
                            {
                                ...
                                Helpers.DrawBezier(EEMod.Instance.Assets.Request<Texture2D>("Textures/Vine").Value, Color.White, ChainConneccPos, LastChainConneccPos, Mid, 0.6f, MathHelper.PiOver2, true);
                                Helpers.DrawBezier(EEMod.Instance.Assets.Request<Texture2D>("Textures/VineLight").Value, Color.White, ChainConneccPos + addOn, LastChainConneccPos + addOn, Mid + addOn, 8f, (lerp1.X < lerp2.X) ? MathHelper.PiOver2 * 3 : MathHelper.PiOver2, true);
                                Helpers.DrawBezier(EEMod.Instance.Assets.Request<Texture2D>("Textures/VineLightGlow").Value, Color.White * Math.Abs((float)Math.Sin(Main.GameUpdateCount / 200f + ChainConneccPos.X)), ChainConneccPos + addOn, LastChainConneccPos + addOn, Mid + addOn, 8f, (lerp1.X < lerp2.X) ? MathHelper.PiOver2 * 3 : MathHelper.PiOver2);
