...
namespace Prophecy.NPCs    //We need this to basically indicate the folder where it is to be read from, so you the texture will load correctly
{
        ...
        public override void AI()
        {
            ...
            if (projectile.ai[0] > 1f)  //this defines where the flames starts
            {
                ...
                for (int i = 0; i<8; i++)    //this defines how many dust to spawn
                {
                    ...
                    //  int dust2 = Dust.NewDust(new Vector2(projectile.position.X, projectile.position.Y), projectile.width, projectile.height, DustID.BlueCrystalShard, projectile.velocity.X * 1.2f, projectile.velocity.Y * 1.2f, DustID.BlueCrystalShard, new Color(255, 255, 153), 1); //this defines the flames dust and color parcticles, like when they fall thru ground, change DustID to wat dust you want from Terraria
                }
            }
            else
            {
